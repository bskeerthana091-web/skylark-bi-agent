...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 p-2 md:p-3 md:px-6">
      <div className="floating-window p-2.5 md:px-4 md:py-2.5 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 shadow-2xl backdrop-blur-xl border border-gray-800/80 w-full max-w-full overflow-hidden">
        
        {/* Top Row / Desktop Left Section: App Logo, Title & Mobile Action Buttons */}
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-brand-500/20 text-brand-300 border border-brand-500/30 whitespace-nowrap">
                  FLOATING OS
                </span>
              </div>
              <p className="text-[10px] text-gray-400 hidden lg:block">Monday.com Business Intelligence Assistant</p>
            </div>
          </div>

          {/* Quick Action Badges on Mobile (< md) */}
          <div className="flex items-center gap-1.5 md:hidden shrink-0">
            <button
              onClick={onOpenDataQuality}
              title="Data Hygiene Audit"
              className="p-1.5 rounded-xl bg-gray-900/80 border border-gray-800 text-emerald-400 text-[11px] font-bold flex items-center gap-1"
            >
            ...
            <button
              onClick={onResetChat}
              title="Reset Session"
              className="p-1.5 rounded-xl bg-gray-900/80 border border-gray-800 text-gray-400"
            >
        ...
        {/* Navigation View Selector Dock */}
        <div className="flex items-center gap-1 bg-gray-950/80 p-1 rounded-2xl border border-gray-800/80 shadow-inner w-full md:w-auto">
          <button
            onClick={() => setActiveView('chat')}
            className={`flex-1 md:flex-none flex items-center justify-center gap-1.5 px-3 md:px-4 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
              activeView === 'chat'
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          ...
          >
            ...
            <span className="text-gray-400 font-medium whitespace-nowrap">Hygiene:</span>
          ...
          <button
            onClick={onOpenMondayConfig}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-semibold transition-all whitespace-nowrap ${
              config.apiToken
          ...
          <button
            onClick={onOpenLeadership}
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-bold shadow-md shadow-purple-600/30 transition-all whitespace-nowrap"
          >
            <FileText className="w-3.5 h-3.5" />
